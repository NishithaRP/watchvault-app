...
export default function MediaList({ category, userId, onAdd, defaultStatus }) {
  ...
  const needsSeasons = category && ['series', 'anime', 'animation', 'donghua'].includes(category)
  ...
  const pageTitle = isAllCategories
    ? (defaultStatus ? STATUS_LABELS_MAP[defaultStatus] : 'All Entries')
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '24px', alignItems: 'center' }}>
        ...
        {/* Category filter when showing all */}
        {isAllCategories && (
      ...
      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '16px' }}>
          {[...Array(8)].map((_, i) => <div key={i} className="card" style={{ height: '280px', animation: 'pulse 1.5s infinite' }} />)}
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 40px', background: 'var(--bg-card)', borderRadius: '16px', border: '1px dashed var(--border-light)' }}>
          ...
          {items.length === 0 && (
            <button className="btn btn-primary" onClick={onAdd}><Plus size={16} /> Add Entry</button>
          )}
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '16px' }}>
    ...
    <div className="card" style={{ position: 'relative' }}
      ...
      <div style={{ height: '200px', background: 'var(--bg-secondary)', position: 'relative', overflow: 'hidden' }}>
        {item.image_url
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Film size={36} style={{ color: 'var(--text-muted)' }} /></div>
        }
        <div style={{ position: 'absolute', top: '8px', left: '8px' }}>
          <span className={`badge ${statusCfg?.class}`} style={{ fontSize: '10px', padding: '2px 7px' }}>{statusCfg?.label}</span>
        </div>
        {showCategory && (
          <div style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(0,0,0,0.75)', borderRadius: '4px', padding: '2px 6px', fontSize: '10px', color: 'var(--gold)', fontWeight: 600 }}>
            {CATEGORY_CONFIG[item.category]?.label}
          </div>
        )}
        {hovered && (
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', animation: 'fadeIn 0.15s ease' }}>
            <button onClick={onEdit} style={{ padding: '8px', borderRadius: '8px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', cursor: 'pointer' }}><Edit2 size={15} /></button>
            <button onClick={onDelete} style={{ padding: '8px', borderRadius: '8px', background: 'rgba(230,57,70,0.2)', border: '1px solid rgba(230,57,70,0.4)', color: 'var(--accent)', cursor: 'pointer' }}><Trash2 size={15} /></button>
          </div>
        )}
      </div>
      <div style={{ padding: '10px 12px' }}>
        <div style={{ fontSize: '13px', fontWeight: 600, lineHeight: 1.3, marginBottom: '6px' }}>{item.name}</div>
        ...
        {item.subcategory && (
          <div style={{ marginTop: '6px', display: 'inline-block', fontSize: '10px', color: 'var(--text-muted)', background: 'var(--bg-secondary)', borderRadius: '4px', padding: '2px 6px' }}>
            {item.subcategory}
          </div>
        )}
      </div>
