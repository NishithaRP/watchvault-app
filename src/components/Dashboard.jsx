    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }} className="fade-in">
          ...
          {Object.entries(STATUS_CONFIG).map(([status, config]) => {
            ...
            return (
              <button key={status} onClick={() => count > 0 && onStatusClick(status)}
                className="card"
                style={{
                  padding: '18px', textAlign: 'left', width: '100%',
                ...
                {count > 0 && (
                  <div style={{ fontSize: '11px', color: config.color, marginTop: '4px' }}>Click to view →</div>
                )}
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px' }}>
          {Object.entries(CATEGORY_CONFIG).map(([cat, config]) => {
            ...
            return (
              <button key={cat} onClick={() => onNavigate(tabId)}
              ...
              <div key={item.id} className="card">
                <div style={{ height: '180px', background: 'var(--bg-secondary)', position: 'relative', overflow: 'hidden' }}>
                  {item.image_url
                    ...
                    : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Film size={32} style={{ color: 'var(--text-muted)' }} /></div>
                  }
                ...
                <div style={{ padding: '10px' }}>
                  <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.3 }}>{item.name}</div>
                  {item.rating && (
      ...
      {stats?.total === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 40px', background: 'var(--bg-card)', borderRadius: '16px', border: '1px dashed var(--border-light)' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>Start by adding movies, series, anime, or manhwa you've watched!</p>
        </div>
      )}
    </div>
  )
